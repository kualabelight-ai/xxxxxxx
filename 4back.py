...
class PromptGenerator:
    ...
    def generate_prompts_for_block(self, block, num_prompts, category="", markers=None, marker_rotator=None):
        ...
        for prompt_num in range(num_prompts):
            # Подготавливаем контекст для "other" блоков ВКЛЮЧАЯ block_id
            context = {
    ...
    def get_weighted_ai_value(self, block_id, var_name, context):
        ...
        # Формируем правильный контекст поиска в зависимости от типа
        search_context = {}

        # Определяем тип контекста
        context_type = context.get("тип", "regular")

        if context_type == "other":
            # Для OTHER блоков: категория + block_id
            search_context = {
                "категория": context.get("категория", ""),
                "block_id": context.get("block_id", ""),
                "тип": "other"
            }
        elif context_type == "regular":
            # Для REGULAR характеристик: категория + характеристика
            search_context = {
                "категория": context.get("категория", ""),
                "характеристика": context.get("характеристика", context.get("название_характеристики", "")),
                "тип": "regular"
                # ЗНАЧЕНИЕ НЕ включаем для regular!
            }
        elif context_type == "unique":
            # Для UNIQUE характеристик: категория + характеристика + значение
            search_context = {
                "категория": context.get("категория", ""),
                "характеристика": context.get("характеристика", context.get("название_характеристики", "")),
        ...
        # Получаем инструкции для этого контекста
        instructions = st.session_state.ai_instruction_manager.get_instruction(
            block_id, var_name, search_context
        )

        if instructions:
            # Инструкции уже разбиты на пункты при сохранении
            # Используем их как есть
            all_items = instructions

            if all_items:
                # Создаем ключ для трекинга использования
                # Для каждого типа - свой набор ключевых полей
                if context_type == "other":
                    tracker_key = f"{block_id}:{var_name}:other:{context.get('категория')}:{context.get('block_id')}"
                elif context_type == "regular":
                    tracker_key = f"{block_id}:{var_name}:regular:{context.get('категория')}:{context.get('характеристика')}"
                elif context_type == "unique":
                    tracker_key = f"{block_id}:{var_name}:unique:{context.get('категория')}:{context.get('характеристика')}:{context.get('значение')}"
                else:
                    # Общий fallback
                    import hashlib
                    context_str = json.dumps(search_context, sort_keys=True)
                    context_hash = hashlib.md5(context_str.encode()).hexdigest()[:8]
                    tracker_key = self.usage_tracker.get_key(block_id, var_name, context_hash)
        ...
        # Если нет AI-инструкций для ЭТОГО контекста, используем адаптивные статические значения
        # НЕ ИЩЕМ инструкции для других контекстов!
        ...
        for var_name in block.get("variables", []):
            ...
            if placeholder in template:
                ...
                if var_data and var_data.get("type") == "ai":
                    # Это AI-переменная
                    if block_type == "other":
                        # Для блоков "other" передаем block_id в контекст
                        block_context = {
                            "категория": context.get("категория", ""),
                            "тип": "other",
                        ...
                        block_context = context.copy()  # Копируем, чтобы не менять оригинал
                        # Убедимся, что тип установлен правильно
                        if char_type:
                            block_context["тип"] = char_type
                        else:
                            block_context["тип"] = "regular"
