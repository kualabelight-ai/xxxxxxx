...
def show_ai_variables_overview():
    ...
    if 'user_id' in st.session_state:
        is_admin_user = is_admin(st.session_state['user_id'])

    # Кнопка очистки только для админов
    if is_admin_user:
        st.markdown("---")
...
def show_edit_mode():
    ...
    if st.session_state.get('edit_variable_direct', False):
        ...
        if var_type == 'static':
    # Вызываем функцию напрямую (она должна быть определена в этом же файле)
    ...
    with tab_settings:
        with st.form(key=f"direct_edit_ai_{block_id}_{var_name}"):
            ...
            with col1:
                provider = st.selectbox(
                    "AI провайдер",
                    ["openai", "deepseek", "genapi_gemini", "true_gemini"],
                    index=["openai", "deepseek", "genapi_gemini", "true_gemini"].index(
                        var_data.get("ai_provider", "deepseek")
                    ) if var_data.get("ai_provider") in ["openai", "deepseek", "genapi_gemini", "true_gemini"] else 1,
                    format_func=lambda x: {
                        "openai": "OpenAI / GPT",
                        "deepseek": "DeepSeek",
                        "genapi_gemini": "Gemini через GenAPI",
                        "true_gemini": "True Gemini (VPN)"
                    }.get(x, x)
                )
...
def show_dynamic_variable_info(var_name):
    """Информация о динамической переменной"""
    st.subheader(f"🌀 Динамическая переменная: `{var_name}`")
    st.info("Динамические переменные настраиваются централизованно и доступны во всех блоках")
    ...
    if 'dynamic_var_manager' in st.session_state:
        ...
        if st.button("⚙️ Перейти к редактору динамических переменных", use_container_width=True):
    ...
    if st.button("🚀 Запустить генерацию AI-инструкций", type="primary",
        ...
        with st.spinner("Генерация AI-инструкций..."):
            ...
            for idx, char in enumerate(selected_chars):
                ...
                # ВАЖНО: Проверяем соответствие типа блока и типа характеристики
                if block_char_type == "regular" and is_unique:
                    st.caption(f"⏭️ Пропущена unique характеристика: {char_name} (блок для regular)")
                    continue
                elif block_char_type == "unique" and not is_unique:
                    st.caption(f"⏭️ Пропущена regular характеристика: {char_name} (блок для unique)")
                    continue
                ...
                if is_unique:
                    ...
                    for value_item in values:
                        ...
                        context = {
                            "категория": category.strip(),  # Добавьте .strip()
...
def batch_generate_for_characteristic(block_id, var_name, var_data, block, provider=None):
    ...
    # Получаем тип блока (regular/unique) из настроек блока
    block_char_type = block.get("settings", {}).get("characteristic_type", "regular")
    ...
    for char in characteristics:
        char_id = char.get('char_id')
        char_name = char.get('char_name')
        is_unique = char.get('is_unique', False)  # Тип характеристики из данных
        values = char.get('values', [])

        # ВАЖНО: Проверяем соответствие типа блока и типа характеристики
        # Если блок для regular, а характеристика unique - пропускаем
        # Если блок для unique, а характеристика regular - пропускаем
        if block_char_type == "regular" and is_unique:
            continue  # Пропускаем unique характеристики для regular блоков
        elif block_char_type == "unique" and not is_unique:
            continue  # Пропускаем regular характеристики для unique блоков
        ...
        if is_unique:
            ...
            for value_item in values:
                ...
                context = {
                    ...
                    "характеристика": char_name,
                    "значение": value,
                    "тип": "unique",  # Важно: указываем тип unique
                    "block_id": block_id,
                    "var_name": var_name
                }
                try:
            ...
            context = {
                "категория": category,
                "характеристика": char_name,
                "тип": "regular",  # Важно: указываем тип regular
                "block_id": block_id,
                "var_name": var_name
            }
            try:
