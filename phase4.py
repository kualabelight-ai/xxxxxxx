...
def main():
    ...
    # В разделе инициализации переменных сессии
    if 'phase4_global_other_prompts' not in st.session_state:
        st.session_state.phase4_global_other_prompts = 3
    local_css()
...
def show_generation_mode(phase1_data, category, markers):
    ...
    if other_blocks:
        with st.expander("📝 Настройка других блоков (заголовок, описание, применение и т.д.)", expanded=True):
            ...
            # Глобальная настройка для всех блоков (как у характеристик)
            col_global1, col_global2 = st.columns([3, 1])
            with col_global1:
                st.markdown("**Глобальная настройка для всех блоков:**")
            with col_global2:
                global_other_prompts = st.number_input(
                    "Промптов на блок:",
                    min_value=0,
                    max_value=200,
                    value=st.session_state.get('phase4_global_other_prompts', 3),
                    key="global_other_prompts_input",
                    label_visibility="collapsed"
                )

                if global_other_prompts != st.session_state.get('phase4_global_other_prompts', 3):
                    st.session_state.phase4_global_other_prompts = global_other_prompts
                    # Применяем глобальную настройку ко всем блокам
                    for block_id in other_blocks.keys():
                        if block_id not in st.session_state.phase4_other_blocks_settings:
                            st.session_state.phase4_other_blocks_settings[block_id] = {
                                'enabled': True,
                                'prompts_count': global_other_prompts
                            }
                        else:
                            st.session_state.phase4_other_blocks_settings[block_id]['prompts_count'] = global_other_prompts
                    st.rerun()
            ...
            for block_id, block in other_blocks.items():
                ...
                if block_id not in st.session_state.phase4_other_blocks_settings:
                    st.session_state.phase4_other_blocks_settings[block_id] = {
                        'enabled': True,
                        'prompts_count': st.session_state.get('phase4_global_other_prompts', 3)
                    }
