...
class Phase6Interface:
    ...
    def _display_postprocessing_results(self):
        """Отображает результаты постобработки: метрики сверху, таблица внизу"""
        results = st.session_state.get('postprocessing_results', {})
        if not results:
            return

        # Метрики всегда видимы
        ...
        # Детальная таблица в сворачиваемом блоке
    ...
    def _apply_postprocessing(self, block_id: str = None):
        ...
        for block in blocks:
            ...
            if symbols:
                new_text, removed_symbols, new_specials = self.text_processor.remove_special_symbols(new_text, symbols)
                if removed_symbols:
            ...
            # 4. ПОЛНАЯ ПРОВЕРКА ОШИБОК (пересчитываем всё заново)
            new_errors = []

            # 4.1 Проверка regular-блоков
            if block.block_type == 'regular':
                # Проверка наличия {prop ...}
                if not re.search(r'\{prop\s+[^}]+\}', new_text):
                    new_errors.append({
                        'type': ErrorType.MISSING_VARIABLE.value,
                        'message': "В regular-блоке отсутствует переменная {prop ...}"
                    })

                # Проверка наличия значения в скобках (если characteristic_value есть)
                if block.characteristic_value:
                    bracket_errors = self.text_processor.check_regular_brackets(new_text, block.characteristic_value)
                    new_errors.extend(bracket_errors)

            # 4.2 Проверка non-regular блоков
            else:
                # Проверка на наличие квадратных скобок
                ...
                # Проверка на неизвестные переменные в скобках [variable]
                matches = self.text_processor.pattern.finditer(new_text)
                for match in matches:
                    var_name = match.group(1).strip()
                    var_lower = var_name.lower()
            ...
            # 4.3 Проверка на спецсимволы (которые не были удалены)


            # 5. Если есть HTML, проверяем и его на запрещённый markdown
            if block.html_text:
    # Удаляем содержимое HTML-тегов
                text_without_tags = re.sub(r'<[^>]*>', '', block.html_text)

                # Проверяем наличие ** или * вне списков
                ...
                for line in lines:
                    ...
                    # Пропускаем строки, которые являются маркерами списка
                    ...
                    # Проверяем наличие * (не в начале и не как часть слова)
                    if re.search(r'(?<!\w)\*(?!\w)', line) or re.search(r'(?<!\w)\*', line):
                        has_forbidden = True
                        break
                ...
                if has_forbidden:
                    new_errors.append({
                        'type': ErrorType.FORBIDDEN_MARKDOWN.value,
                        'message': "В HTML найдено запрещённое форматирование '*'. Разрешены только маркеры списков в начале строки."
                    })
            ...
            # Важно: заменяем все ошибки новыми, а не добавляем
            ...
            if new_errors:
                ...
                # Если ошибок нет, но блок был вручную исправлен
            ...
            # Если HTML изменился (например, при ручном редактировании), не перезаписываем его
            # Постобработка не трогает HTML, только текст
        ...
        if processed_count:
            ...
            if total_errors:
                st.warning(f"⚠️ Найдено {total_errors} ошибок. Проверьте блоки.")
        else:
    ...
    def _auto_insert_regular_blocks(self, block_id: str = None):
        ...
        for block in blocks:
            ...
            if has_error:
                ...
                # Очищаем предыдущие ошибки этого типа, если были
    ...
    def _apply_generate_html(self, block_id: str = None):
        ...
        for block in blocks:
            ...
            # Добавляем ошибки форматирования в блок
            for err in errors:
            ...
            trans = TextTransformation(
                ...
                severity=SeverityLevel.ERROR if errors else SeverityLevel.INFO,
                user="user"
            )
            registry.add(trans)
            generated += 1
        ...
        if total_errors:
            st.error(f"❌ Найдено {total_errors} ошибок форматирования (запрещённые ** или *)")
    ...
    def _reset_state(self):
        ...
        keys_to_clear = [
            ...
            'ui_state',
            'postprocessing_results',  # <-- ДОБАВИТЬ ЭТУ СТРОКУ
            'variables_replaced'       # <-- И ЭТУ, если нужно сбрасывать флаг замены переменных
        ]
    ...
    def _display_top_panel(self):
        """Отображает настройки в сайдбаре"""
        with st.sidebar:
    ...
    def _render_editable_block(self, block: FragmentBlock):
        ...
        with st.container(border=True):
            ...
            # --- ИСПРАВЛЕНО: поле для HTML с проверкой существования ключа ---
            html_key = f"edit_html_{block.id}"

            # Важно: НЕ обновляем session_state, если виджет уже существует
            # Вместо этого используем value из блока, но НЕ присваиваем в session_state

            with st.expander("🌐 HTML версия (редактируемая)", expanded=False):
                # Создаём виджет, но не изменяем session_state после создания
                edited_html = st.text_area(
                    "HTML код",
                    value=block.html_text,  # Берём значение напрямую из блока, а не из session_state
                    height=150,
                    key=html_key,
                    label_visibility="collapsed"
                )
            ...
            with col_save:
                if st.button("💾 Сохранить", key=f"save_{block.id}", use_container_width=True):
                    ...
                    block.html_text = st.session_state[html_key]  # Берём из session_state, куда его записал виджет
                    block.last_modified = datetime.now()
